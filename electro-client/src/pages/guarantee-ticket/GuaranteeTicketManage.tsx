...
function GuaranteeTicketManage() {
  ...
  const entityDetailTableRowsFragment = (entity: GuaranteeResponse) => (
    <>
      <tr>
        <td>{GuaranteeTicketConfigs.properties.id.label}</td>
        <td>{entity.id}</td>
      </tr>
      <tr>
        <td>{GuaranteeTicketConfigs.properties.createdAt.label}</td>
        <td>{DateUtils.isoDateToString(entity.createdAt)}</td>
      </tr>
      <tr>
        <td>{GuaranteeTicketConfigs.properties.updatedAt.label}</td>
        <td>{DateUtils.isoDateToString(entity.updatedAt)}</td>
      </tr>
      <tr>
        <td>{GuaranteeTicketConfigs.properties.name.label}</td>
        <td>{entity.name}</td>
      </tr>
      <tr>
        <td>{GuaranteeTicketConfigs.properties.description.label}</td>
        <td style={{ maxWidth: 300 }}>{entity.description}</td>
      </tr>
      <tr>
        <td>{GuaranteeTicketConfigs.properties.status.label}</td>
        <td>{guaranteeStatusBadgeFragment(entity.status)}</td>
      </tr>
    </>
  );
  ...
  return (
    <Stack>
      <ManageHeader>
        <ManageHeaderTitle
          titleLinks={GuaranteeTicketConfigs.manageTitleLinks}
          title={GuaranteeTicketConfigs.manageTitle}
        />
        <ManageHeaderButtons
          listResponse={listResponse}
          resourceUrl={GuaranteeTicketConfigs.resourceUrl}
          resourceKey={GuaranteeTicketConfigs.resourceKey}
        />
      </ManageHeader>
...
export default GuaranteeTicketManage;
